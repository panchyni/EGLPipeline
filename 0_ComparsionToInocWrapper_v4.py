## Change in version 4 ##
# -Add test of difference in allele freq across windows by Mann-Whitney U
# -Need to merge with Fisher's Extact Test function to reduce number of file I/Os (lazy ATM)
...
def MannWhitneyU_AlleleFreqDelta(CtlFiles,TrtFiles,wsize,witer,minSNP,file_root):
    '''Get allele frequecnes at each site and compare neighboring windows via Mann-Whinty U test

       Return the name of p-value output files
    '''
    ...
    for cf in CtlFiles:
        ...
        for cf_ln in cf_lines:
            split_ln = cf_ln.split("\t")
            position = int(split_ln[1])
            snp_freq = float(split_ln[-1])
            ctl_dict[cf][position] = [snp_freq]
            all_positions.append(position)
    ...
    ctl_results= {}
    trt_results = {}
    for w in window_end_list[0:-1]:
        
        # Make an entry for both sets of files
        ctl_results[w] = {}
        trt_results[w] = {}
                
        for cf in CtlFiles:
            cf_current_freqs = [ctl_dict[cf][i] for i in ctl_dict[cf].keys() if i < w and i >= w-wsize]
            cf_next_freqs = [ctl_dict[cf][i] for i in ctl_dict[cf].keys() if i < w+wsize and i >= w]

            if len(cf_current_freqs) < minSNP or len(cf_next_freqs) < minSNP:
                ctl_results[w][cf] = "NA"
            else:
                ctl_results[w][cf] = stats.mannwhitneyu(cf_current_freqs,cf_next_freqs)[1]
            
        for tf in TrtFiles:
            tf_current_freqs = [trt_dict[tf][i] for i in trt_dict[tf].keys() if i < w and i >= w-wsize]
            tf_next_freqs = [trt_dict[tf][i] for i in trt_dict[tf].keys() if i < w+wsize and i >= w]

            if len(tf_current_freqs) < minSNP or len(tf_next_freqs) < minSNP:
                trt_results[w][tf] = "NA"
            else:
                trt_results[w][tf] = stats.mannwhitneyu(tf_current_freqs,tf_next_freqs)[1]

    ctl_header = "1st_Window_Start\t2nd_Window_Start\t" + "\t".join(CtlFiles) + "\n"
    trt_header = "1st_Window_Start\t2nd_Window_Start\t" + "\t".join(TrtFiles) + "\n"
    
    ctl_outlines = []
    for w in window_end_list[0:-1]:
        ctl_outlines.append(str(w-wsize) + "\t" + str(w) + "\t" + "\t".join([str(ctl_results[w][cf]) for cf in CtlFiles]) + "\n")

    trt_outlines = []
    for w in window_end_list[0:-1]:
        trt_outlines.append(str(w-wsize) + "\t" + str(w) + "\t" + "\t".join([str(trt_results[w][tf]) for tf in TrtFiles]) + "\n")
      
    ctl_output = open(file_root + ".MannWhitneyU_Control","w")
    ctl_output.write(ctl_header)
    ctl_output.write("".join(ctl_outlines))
    ctl_output.close()

    trt_output = open(file_root + ".MannWhitneyU_Treatment","w")
    trt_output.write(trt_header)
    trt_output.write("".join(trt_outlines))
    trt_output.close()

    return file_root + ".MannWhitneyU_Control", file_root + ".MannWhitneyU_Treatment"
...
def main():
    ...
    print "Step2b Finished [" + FormatTime(time.time() - start_timer) + "]"

    print "Step2c Run MannWhitneyU on Allele Frequenes [" + FormatTime(time.time() - start_timer) + "]"

    MWU_CtlFile, MWU_TrtFile =  MannWhitneyU_AlleleFreqDelta(CtlMergedFiles,TrtMergedFiles,int(parameter_dictionary['Wsize']),int(parameter_dictionary['Witer']),int(parameter_dictionary['minSNP']),parameter_dictionary['MatrixFile'])

    print "MannWhitneyU-test - Control Windows File:" + MWU_CtlFile
    print "MannWhitneyU-test - Treatment Windows File:" + MWU_TrtFile

    print "Step2c Finished [" + FormatTime(time.time() - start_timer) + "]"
